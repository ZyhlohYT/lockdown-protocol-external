...
// XOR key for obfuscation - changes each run
static uint64_t __xor_key = 0;

// Obfuscated function pointer storage
static uint64_t __obfuscated_ptr = 0;

// Function to decode the obfuscated pointer when needed
tProcessEvent get_proc_handler() {
    if (__obfuscated_ptr == 0) return nullptr;
    
    // Decode the XORed pointer
    uint64_t decoded = __obfuscated_ptr ^ __xor_key;
    return reinterpret_cast<tProcessEvent>(decoded);
}

// Initialize system components with obfuscation
void initialize_process_event() {
    // Generate a random XOR key
    srand(static_cast<unsigned int>(time(nullptr)));
    __xor_key = (static_cast<uint64_t>(rand()) << 32) | rand();
    
    // Add some junk calculations to confuse static analysis
    uint64_t junk_val = 0;
    for (int i = 0; i < (rand() % 10) + 5; i++) {
        junk_val += (static_cast<uint64_t>(rand()) * i);
    }
    
    // Calculate the address using a more complex approach
    uintptr_t base_addr = mem::module_base;
    uintptr_t offset = protocol::engine::PROCESSEVENT;
    
    // Split the calculation to make it harder to follow
    uintptr_t part1 = base_addr + (offset / 2);
    uintptr_t part2 = offset - (offset / 2);
    uintptr_t process_event_address = part1 + part2;
    
    // Add timing variation to avoid pattern detection
    std::this_thread::sleep_for(std::chrono::milliseconds(rand() % 10 + 1));
    
    // Store the obfuscated pointer
    __obfuscated_ptr = static_cast<uint64_t>(process_event_address) ^ __xor_key;
    
    // Set the global pointer through our getter
    globals::process_event = get_proc_handler();
    
    // Use generic success message to avoid revealing what was initialized
    if (globals::process_event) {
        std::cout << "System component initialized successfully" << std::endl;
    }
    else {
        std::cerr << "Failed to initialize system component" << std::endl;
    }
}
...
static void render_callback() {
				...
				if (ghost_distance > 2) { // If ghost moved away from the body
					if (player_ghost) {
						...
						if (util::w2s(ghostPosition, last_frame_cached.pov, ghost_screen_position)) {
							...
							vector3 original_position = ghost_screen_position;
							auto distance = CalculateDistance(local_mec->get_net_location(), ghostPosition);

							// Draw distance on first line if enabled
							if (player_distance) {
								// Format distance text with dash and space
								std::string distance_text = "- " + distance + "m";
								int distance_width = distance_text.length() * 5;
								
								// Center the distance text
								ghost_screen_position.x -= distance_width / 2;
								
								overlay->draw_text(ghost_screen_position, ghostcolor, distance_text.c_str(), true);
								
								// Move down for name
								ghost_screen_position = original_position;
								ghost_screen_position.y += 15;
							}

							// Format and draw player name
							std::string ghost_name = "[" + name + "][G]";
					...
					if (distanceDouble <= esp_max_distance) {
						...
						if (util::w2s(position, last_frame_cached.pov, screen_position)) {
							// Store original screen position
							vector3 original_position = screen_position;
							...
							if (player_distance) {
								...
								// Center the distance text
								screen_position.x -= distance_width / 2;
								
								overlay->draw_text(screen_position, color, distance_text.c_str(), true);
								
								// Move down for name
								screen_position = original_position;
							...
							if (player_inventory) {
								...
								auto format_item_state = [](const std::string& item_name, const auto& item_state) -> ItemStateInfo {
									...
									if (item_name == "VENT FILTER") {
										info.text = std::to_string(item_state.Value_8) + "%";
										...
										const char* cassette_titles[] = {
											...
											"KHARMA", "WORSHIP", "ROYALTY", "GRAVE"
										};
										if (item_state.Value_8 >= 0 && item_state.Value_8 < IM_ARRAYSIZE(cassette_titles)) {
											info.text = std::string(cassette_titles[item_state.Value_8]);
										}
										else {
											info.text = "TAPE";
										}
										...
										const char* rice_options[] = { "White", "Brown", "Black" };
										...
										std::string rice = (rice_value > 0 && rice_value <= IM_ARRAYSIZE(rice_options)) ? rice_options[rice_value - 1] : "?";
										std::string fish = (fish_value > 0 && fish_value <= IM_ARRAYSIZE(fish_options)) ? fish_options[fish_value - 1] : "?";
										std::string container = (container_value > 0 && container_value <= IM_ARRAYSIZE(container_colors)) ? container_colors[container_value - 1] : "?";
										...
										const char* egg_types[] = { "Yellow", "Blue", "Green", "Pink", "Tan", "Red", "Orange", "D-Blue" };
								...
								// Prepare the item name display strings with special formatting for all items
								std::string hand_item_display_text = "";
								if (!mec_hand_item_name.empty()) {
									// Start with the basic item name
									hand_item_display_text = "[" + mec_hand_item_name;
									
									// Add state information if present
									if (!hand_item_info.text.empty()) {
										hand_item_display_text += ":" + hand_item_info.text;
										// Clear the state text since we're including it in the item display
										hand_item_info.text = "";
									}
									
									// Close the bracket
									hand_item_display_text += "]";
								}
								
								std::string bag_item_display_text = "";
								if (!mec_bag_item_name.empty()) {
									// Start with the basic item name
									bag_item_display_text = "[" + mec_bag_item_name;
									
									// Add state information if present
									if (!bag_item_info.text.empty()) {
										bag_item_display_text += ":" + bag_item_info.text;
										// Clear the state text since we're including it in the item display
										bag_item_info.text = "";
									}
									
									// Close the bracket
									bag_item_display_text += "]";
								}
								...
								// Move down for inventory display
								screen_position.y += 15;
								
								// Create a single combined string for display
								std::string combined_text = "";
								
								// When both items are present, combine them with a separator
								if (!hand_item_display_text.empty() && !bag_item_display_text.empty()) {
									combined_text = hand_item_display_text + " | " + bag_item_display_text;
								}
								// Only hand item is present
								else if (!hand_item_display_text.empty()) {
									combined_text = hand_item_display_text;
								}
								// Only bag item is present
								else if (!bag_item_display_text.empty()) {
									combined_text = bag_item_display_text;
								}
								
								// Display the combined text if not empty
								if (!combined_text.empty()) {
									// Center the text
									vector3 text_position = screen_position;
									text_position.x = original_position.x - (combined_text.length() * 5 / 2);
									
									// Use a neutral color for the combined text (white)
									ImVec4 text_color(1.0f, 1.0f, 1.0f, 1.0f);
									overlay->draw_text(text_position, ImGui::ColorConvertFloat4ToU32(text_color), combined_text.c_str(), true);
								}
								
								// Move down for item states if any exist
								screen_position.y += 15;
								
								// Display states if they exist and weren't incorporated into the item display
								if (!hand_state_text.empty() || !bag_state_text.empty()) {
									// Both states are present
									if (!hand_state_text.empty() && !bag_state_text.empty()) {
										// Calculate center position for the combined text
										vector3 hand_state_position = screen_position;
										hand_state_position.x = original_position.x - (hand_state_text.length() * 5 / 2) - 10; // Move hand state left
										
										// Draw hand state with its color
										overlay->draw_text(hand_state_position, hand_item_info.color, hand_state_text.c_str(), true);
										
										// Draw separator in white color
										vector3 separator_position = screen_position;
										separator_position.x = original_position.x - 5; // Center the separator
										overlay->draw_text(separator_position, ImGui::ColorConvertFloat4ToU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), "|", true);
										
										// Draw bag state with its color
										vector3 bag_state_position = screen_position;
										bag_state_position.x = original_position.x + 10; // Move bag state right
										overlay->draw_text(bag_state_position, bag_item_info.color, bag_state_text.c_str(), true);
									}
									// Only hand state is present
									else if (!hand_state_text.empty()) {
										// Center the single state
										vector3 hand_state_position = screen_position;
										hand_state_position.x = original_position.x - (hand_state_text.length() * 5 / 2);
										overlay->draw_text(hand_state_position, hand_item_info.color, hand_state_text.c_str(), true);
									}
									// Only bag state is present
									else if (!bag_state_text.empty()) {
										// Center the single state
										vector3 bag_state_position = screen_position;
										bag_state_position.x = original_position.x - (bag_state_text.length() * 5 / 2);
										overlay->draw_text(bag_state_position, bag_item_info.color, bag_state_text.c_str(), true);
									}
								}
	...
	for (auto item : world_item_cache) {
		...
		if (item_name == "PISTOL" || item_name == "REVOLVER" || item_name == "SHORTY" || item_name == "SMG" || item_name == "RIFLE" || item_name == "SHOTGUN") {
			...
			if (weapon_esp) {
				// Skip weapons with 0 ammo if hide_empty_weapons is enabled
				if (hide_empty_weapons && weapon_item_state && item_value <= 0) {
					continue;
				}
				...
				if (distanceDouble <= esp_max_distance) {
					...
					if (util::w2s(position, last_frame_cached.pov, screen_position)) {
						...
						// Format weapon text
						std::string weapon_text = "[" + item_name + "]";
						if (weapon_item_state) {
							weapon_text += ":" + std::to_string(item_value);
						}
						
						// Calculate weapon text width
						int weapon_width = weapon_text.length() * 5;
						
						// Center weapon text
						screen_position.x -= weapon_width / 2;
						
						// Draw weapon name and ammo on first line
						overlay->draw_text(screen_position, weapon_esp_color, weapon_text.c_str(), true);
						
						// Draw distance on second line if enabled
						if (weapon_distance) {
							// Format distance text
							std::string distance_text = "- " + distance + "m";
							int distance_width = distance_text.length() * 5;
							
							// Reset to original position, then center distance text
							screen_position = original_position;
							screen_position.y += 15; // Move down to next line
							screen_position.x -= distance_width / 2; // Center distance text
							
							overlay->draw_text(screen_position, weapon_esp_color, distance_text.c_str(), true);
						}
					}
				}
			}
			...
			if (weapon_esp) {
				...
				if (distanceDouble <= esp_max_distance) {
					...
					if (util::w2s(position, last_frame_cached.pov, screen_position)) {
						...
						// Draw weapon name on first line
						overlay->draw_text(screen_position, weapon_esp_color, weapon_text.c_str(), true);
				...
				if (distanceDouble <= esp_max_distance) {
					...
					if (util::w2s(position, last_frame_cached.pov, screen_position)) {
						...
						if (item_name == "GAZ BOTTLE") {
							...
							// Format item text
							std::string item_text = "[" + item_name + "]";
							
							// Calculate text width
							int text_width = item_text.length() * 5;
							screen_position.x -= text_width / 2; // Center text
							
							// Draw item name on first line
							overlay->draw_text(screen_position, gaz_bottle_esp_color, item_text.c_str(), true);
							...
							if (primary_distance) {
								...
								// Reset to original position, then center distance text
								vector3 distance_pos = original_position;
								distance_pos.y += 15; // Move down to next line
								distance_pos.x -= distance_width / 2; // Center distance text
								
								overlay->draw_text(distance_pos, gaz_bottle_esp_color, distance_text.c_str(), true);
								
								// Move the position down for item state
							...
							// Draw item name on first line
							overlay->draw_text(screen_position, access_card_esp_color, item_text.c_str(), true);
							...
							if (primary_distance) {
								...
								overlay->draw_text(distance_pos, access_card_esp_color, distance_text.c_str(), true);
							}
						}
							...
							// Draw item name on first line
							overlay->draw_text(screen_position, vent_filter_esp_color, item_text.c_str(), true);
							...
							if (primary_distance) {
								...
								overlay->draw_text(distance_pos, vent_filter_esp_color, distance_text.c_str(), true);
							...
							// Format item text based on rice type
							std::string item_text;
							if (item_value == 1) {
								item_text = "[WHITE RICE]";
							}
							else if (item_value == 2) {
								item_text = "[BROWN RICE]";
							}
							else if (item_value == 3) {
								item_text = "[BLACK RICE]";
							}
							else {
								item_text = "[" + item_name + "]";
							}
							...
							// Draw item name on first line
							overlay->draw_text(screen_position, rice_esp_color, item_text.c_str(), true);
							...
							if (primary_distance) {
								...
								overlay->draw_text(distance_pos, rice_esp_color, distance_text.c_str(), true);
							}
						}
						else if (item_name == "PACKAGE") {
							// Store original screen position
							...
							// Draw item name on first line
							overlay->draw_text(screen_position, package_esp_color, item_text.c_str(), true);
							...
							if (primary_distance) {
								...
								overlay->draw_text(distance_pos, package_esp_color, distance_text.c_str(), true);
							...
							// Format item text based on sample type
							std::string item_text;
							if (item_value == 1) {
								item_text = (item_time == 1) ? "[GREEN MIX SAMPLE]" : "[GREEN SAMPLE]";
							}
							else if (item_value == 2) {
								item_text = (item_time == 1) ? "[YELLOW MIX SAMPLE]" : "[YELLOW SAMPLE]";
							}
							else if (item_value == 3) {
								item_text = (item_time == 1) ? "[BLUE MIX SAMPLE]" : "[BLUE SAMPLE]";
							}
							else if (item_value == 4) {
								item_text = (item_time == 1) ? "[WHITE MIX SAMPLE]" : "[WHITE SAMPLE]";
							}
							else if (item_value == 5) {
								item_text = "[RED SAMPLE]"; // Red doesn't mix, so no mix logic
							}
							else {
								item_text = "[" + item_name + "]";
							}
							...
							// Draw item name on first line
							overlay->draw_text(screen_position, sample_esp_color, item_text.c_str(), true);
							...
							if (primary_distance) {
								...
								overlay->draw_text(distance_pos, sample_esp_color, distance_text.c_str(), true);
							}
						}
					}
				}
		...
		if (item_name == "FUSE" || item_name == "BATTERY" || item_name == "SCREW DRIVER" || item_name == "CONTAINER" || item_name == "EGG" || item_name == "EASTEREGG" || item_name == "DEFIBRILLATOR" || item_name == "REZ" || item_name == "MACHINE PART") {
			...
			if (secondary_object_esp) {
				...
				if (distanceDouble <= esp_max_distance) {
					vector3 screen_position{};
					if (util::w2s(position, last_frame_cached.pov, screen_position)) {
						// Store original screen position for all secondary objects
						vector3 original_position = screen_position;
						...
						if (item_name == "FUSE") {
							// Draw item name on first line
							overlay->draw_text(screen_position, fuse_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, fuse_esp_color, distance_text.c_str(), true);
							...
							// Display fuse color information if enabled
							if (secondary_item_state) {
								...
								// Center the fuse info text
								vector3 info_pos = original_position;
								info_pos.y = screen_position.y; // Use the updated y position
								int info_width = fuse_info.length() * 5;
								info_pos.x -= info_width / 2;
								
								overlay->draw_text(info_pos, fuse_esp_color, fuse_info.c_str(), true);
							}
						}
						else if (item_name == "BATTERY") {
							// Draw item name on first line
							overlay->draw_text(screen_position, battery_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, battery_esp_color, distance_text.c_str(), true);
							...
							// Display battery charge information if enabled
							if (secondary_item_state) {
								std::string charge_info = "[Charge: " + std::to_string(item_value) + "%]";
								
								// Center the charge info text
								vector3 info_pos = original_position;
								info_pos.y = screen_position.y; // Use the updated y position
								int info_width = charge_info.length() * 5;
								info_pos.x -= info_width / 2;
								
								overlay->draw_text(info_pos, battery_esp_color, charge_info.c_str(), true);
							}
						}
						else if (item_name == "SCREW DRIVER") {
							// Draw item name on first line
							overlay->draw_text(screen_position, screw_driver_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, screw_driver_esp_color, distance_text.c_str(), true);
							}
						}
						else if (item_name == "MACHINE PART") {
							// Store original screen position
							...
							// Draw item name on first line
							overlay->draw_text(screen_position, machine_part_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, machine_part_esp_color, distance_text.c_str(), true);
							}
						}
						else if (item_name == "EGG" || item_name == "EASTEREGG") {
							// Draw item name on first line
							overlay->draw_text(screen_position, egg_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, egg_esp_color, distance_text.c_str(), true);
							...
							// Display egg color information if enabled
							if (secondary_item_state) {
								std::string color_info;
								
								switch (item_value) {
									case 0: color_info = "[Yellow]"; break;
									case 1: color_info = "[Blue]"; break;
									case 2: color_info = "[Green]"; break;
									case 3: color_info = "[Pink]"; break;
									case 4: color_info = "[Tan]"; break;
									case 5: color_info = "[Red]"; break;
									case 6: color_info = "[Orange]"; break;
									case 7: color_info = "[Dark Blue]"; break;
									default: color_info = "[Unknown]"; break;
								}
								
								// Center the color info text
								vector3 info_pos = original_position;
								info_pos.y = screen_position.y; // Use the updated y position
								int info_width = color_info.length() * 5;
								info_pos.x -= info_width / 2;
								
								overlay->draw_text(info_pos, egg_esp_color, color_info.c_str(), true);
							}
						}
						else if (item_name == "DEFIBRILLATOR" || item_name == "REZ") {
							// Draw item name on first line
							overlay->draw_text(screen_position, defib_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, defib_esp_color, distance_text.c_str(), true);
							...
							if (secondary_item_state) {
								...
								overlay->draw_text(info_pos, defib_esp_color, charge_info.c_str(), true);
							}
						}
						else if (item_name == "CONTAINER") {
							// Draw item name on first line
							overlay->draw_text(screen_position, container_esp_color, item_text.c_str(), true);
							...
							if (secondary_distance) {
								...
								overlay->draw_text(distance_pos, container_esp_color, distance_text.c_str(), true);
							...
							// Display container state information if enabled
							if (secondary_item_state) {
								...
								if (item_value == -1) {
									...
									if (item_time == 1) {
										state_info = "[White Rice]";
									}
									else if (item_time == 2) {
										state_info = "[Brown Rice]";
									}
									else if (item_time == 3) {
										state_info = "[Black Rice]";
									}
								}
								
								// Center the state info text
								vector3 info_pos = original_position;
								info_pos.y = screen_position.y; // Use the updated y position
								int info_width = state_info.length() * 5;
								info_pos.x -= info_width / 2;
								
								overlay->draw_text(info_pos, container_esp_color, state_info.c_str(), true);
							}
						}
					}
				}
...
int main() {
    ...
    std::ofstream logFile("lockdown_log.txt");
    std::streambuf* oldCout = std::cout.rdbuf();
    std::streambuf* oldCerr = std::cerr.rdbuf();
    std::cout.rdbuf(logFile.rdbuf());
    std::cerr.rdbuf(logFile.rdbuf()); // Also redirect std::cerr
	...
	std::cout << "Protocol Overlay initialized." << std::endl;
