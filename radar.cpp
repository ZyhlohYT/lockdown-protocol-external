...
void radar::draw() {
	if (esp_radar) {
		...
		// Draw radar background with gradient effect
		const float outerRadius = radarSize / 2;
		const float innerRadius = outerRadius * 0.95f;
		
		// Draw outer circle with soft edge
		ImGui::GetForegroundDrawList()->AddCircleFilled(
			toImVec2(radarCenter), outerRadius, IM_COL32(0, 0, 0, 150), 64); // Semi-transparent black
		
		// Draw inner circle with slightly different opacity for depth effect
		ImGui::GetForegroundDrawList()->AddCircleFilled(
			toImVec2(radarCenter), innerRadius, IM_COL32(0, 0, 0, 130), 64);
		
		// Draw outer border with anti-aliased edge
		ImGui::GetForegroundDrawList()->AddCircle(
			toImVec2(radarCenter), outerRadius, IM_COL32(255, 255, 255, 180), 64, 1.5f);
		
		// Draw compass markers (N, E, S, W)
		const float markerDistance = outerRadius * 0.85f;
		const float textSize = 14.0f;
		ImGui::GetForegroundDrawList()->AddText(default_font.im_font, textSize, 
			ImVec2(radarCenter.x - 4, radarCenter.y - markerDistance - textSize), 
			IM_COL32(255, 255, 255, 200), "N");
		ImGui::GetForegroundDrawList()->AddText(default_font.im_font, textSize, 
			ImVec2(radarCenter.x + markerDistance - 4, radarCenter.y - 8), 
			IM_COL32(255, 255, 255, 200), "E");
		ImGui::GetForegroundDrawList()->AddText(default_font.im_font, textSize, 
			ImVec2(radarCenter.x - 4, radarCenter.y + markerDistance - textSize), 
			IM_COL32(255, 255, 255, 200), "S");
		ImGui::GetForegroundDrawList()->AddText(default_font.im_font, textSize, 
			ImVec2(radarCenter.x - markerDistance - 4, radarCenter.y - 8), 
			IM_COL32(255, 255, 255, 200), "W");
		
		// Draw subtle range rings
		ImGui::GetForegroundDrawList()->AddCircle(
			toImVec2(radarCenter), outerRadius * 0.33f, IM_COL32(255, 255, 255, 40), 64, 1.0f);
		ImGui::GetForegroundDrawList()->AddCircle(
			toImVec2(radarCenter), outerRadius * 0.66f, IM_COL32(255, 255, 255, 40), 64, 1.0f);

		// Player indicator (triangle with dot)
		float playerDotSize = 4.0f; // Size of the player's dot
		float playerTriangleSize = 8.0f; // Size of the direction triangle
		
		// Draw player position dot
		ImGui::GetForegroundDrawList()->AddCircleFilled(
			toImVec2(radarCenter), playerDotSize, IM_COL32(255, 255, 255, 255)); // White dot
		
		// Draw player direction indicator (small triangle)
		std::vector<ImVec2> playerTriangle = compute_triangle(radarCenter, playerTriangleSize, camera_yaw);
		ImGui::GetForegroundDrawList()->AddTriangleFilled(
			playerTriangle[0], playerTriangle[1], playerTriangle[2], 
			IM_COL32(255, 255, 0, 255)); // Yellow triangle

		// Cone to represent the player's POV with gradient effect
		...
		const int numSegments = 48; // Increased segments for smoother curvature
		...
		// Draw the FOV cone as a filled polygon with better opacity
		ImGui::GetForegroundDrawList()->AddConvexPolyFilled(
			coneVertices.data(), static_cast<int>(coneVertices.size()), 
			IM_COL32(255, 255, 255, 40)); // White with subtle opacity
			
		// Draw FOV cone border for better definition
		ImVec2 leftEdge = coneVertices[1];
		ImVec2 rightEdge = coneVertices[coneVertices.size() - 1];
		ImGui::GetForegroundDrawList()->AddLine(
			toImVec2(radarCenter), leftEdge, IM_COL32(255, 255, 255, 100), 1.0f);
		ImGui::GetForegroundDrawList()->AddLine(
			toImVec2(radarCenter), rightEdge, IM_COL32(255, 255, 255, 100), 1.0f);
			
		// Draw arc at the edge of FOV for better definition
		ImGui::GetForegroundDrawList()->AddPolyline(
			&coneVertices[1], numSegments, IM_COL32(255, 255, 255, 100), 0, 1.0f);
