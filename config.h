...
namespace config {
	// Hotkeys
	inline int menu_hotkey = VK_INSERT;
	...
	inline int no_recoil_hotkey = VK_F1;
	...
	inline bool hide_empty_weapons = false;
	...
	inline void SaveConfig(const std::string& filename = "config.json") {
		...
		j["hide_empty_weapons"] = hide_empty_weapons;
	...
	inline void LoadConfig(const std::string& filename = "config.json") {
		...
		try {
			...
			// Load hide_empty_weapons if it exists in the config
			if (j.contains("hide_empty_weapons")) {
				j.at("hide_empty_weapons").get_to(hide_empty_weapons);
			}
