...
void menu::draw()
{
	...
	// Animation variables (static to persist between frames)
	static float animTime = 0.0f;
	static float pulseAnimation = 0.0f;
	static float menuAlpha = 0.0f;
	static bool initialAnimation = true;
	
	// Update animations
	animTime += ImGui::GetIO().DeltaTime * 0.5f; // Controls overall animation speed
	pulseAnimation = (std::sin(animTime * 2.0f) + 1.0f) * 0.5f; // Pulsing effect (0.0 to 1.0)
	
	// Fade-in animation when menu opens
	if (initialAnimation) {
		menuAlpha = (menuAlpha + ImGui::GetIO().DeltaTime * 4.0f < 1.0f) ? 
					(menuAlpha + ImGui::GetIO().DeltaTime * 4.0f) : 1.0f; // Manual min function
		if (menuAlpha >= 1.0f) initialAnimation = false;
	} else {
		menuAlpha = 1.0f;
	}

	if (menu_open) {
		// Set up window properties with dynamic scaling based on resolution
		ImVec2 screenSize = ImGui::GetIO().DisplaySize;
		
		// Calculate scale factor based on screen resolution
		// Base resolution is 2560x1440 (your resolution)
		const float BASE_WIDTH = 2560.0f;
		const float BASE_HEIGHT = 1440.0f;
		
		// Calculate scale factors for width and height
		float scaleX = screenSize.x / BASE_WIDTH;
		float scaleY = screenSize.y / BASE_HEIGHT;
		
		// Use the smaller scale factor to ensure menu fits on screen
		float scaleFactor = (scaleX < scaleY) ? scaleX : scaleY;
		
		// Apply scale factor to menu size (1050x700 at 2560x1440)
		const float BASE_MENU_WIDTH = 1050.0f;
		const float BASE_MENU_HEIGHT = 700.0f;
		ImVec2 windowSize = ImVec2(BASE_MENU_WIDTH * scaleFactor, BASE_MENU_HEIGHT * scaleFactor);
		
		// Scale starting position as well
		ImVec2 startPosition = ImVec2(20 * scaleFactor, 20 * scaleFactor);

		// Set window style with more pronounced effects - scaled based on resolution
		ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 12.0f * scaleFactor);
		ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 1.0f * scaleFactor);
		ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(15 * scaleFactor, 15 * scaleFactor));
		ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(10 * scaleFactor, 10 * scaleFactor)); // More spacing between items
		
		// Scale global ImGui font size
		ImGui::GetIO().FontGlobalScale = scaleFactor;
		
		// Set window position and size
		ImGui::SetNextWindowPos(startPosition, ImGuiCond_Once);
		ImGui::SetNextWindowSize(windowSize, ImGuiCond_Once);

		// Set window colors with animated transparency - dark purple theme
		ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.05f, 0.03f, 0.08f, 0.8f * menuAlpha)); // Darker purple background with 20% transparency
		ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.6f + pulseAnimation * 0.2f, 0.2f + pulseAnimation * 0.1f, 0.8f + pulseAnimation * 0.1f, (0.6f + pulseAnimation * 0.4f) * menuAlpha)); // Animated glowing purple border
		ImGui::PushStyleColor(ImGuiCol_TitleBg, ImVec4(0.12f, 0.06f, 0.18f, 0.8f * menuAlpha));
		ImGui::PushStyleColor(ImGuiCol_TitleBgActive, ImVec4(0.18f, 0.09f, 0.25f, 0.8f * menuAlpha));

		// Begin the main window with custom flags - use empty title since we'll draw our own
		// Set fixed size window with no resizing
		ImGui::SetNextWindowSize(windowSize, ImGuiCond_Always); // Force size every frame
		...
		// Store these for later use with the X button
		static ImVec2 lastWinPos = winPos;
		static ImVec2 lastWinSize = winSize;
		
		// Create a grid pattern background - purple theme
		float gridSize = 20.0f;
		float gridAlpha = 0.05f + pulseAnimation * 0.03f; // Slightly more subtle grid
		
		// Horizontal lines
		for (float y = 0; y < winSize.y; y += gridSize) {
			draw_list->AddLine(
				ImVec2(winPos.x, winPos.y + y),
				ImVec2(winPos.x + winSize.x, winPos.y + y),
				IM_COL32(160, 100, 220, static_cast<int>(gridAlpha * 255))
			);
		}
		
		// Vertical lines
		for (float x = 0; x < winSize.x; x += gridSize) {
			draw_list->AddLine(
		...
		// Add animated accent circles in corners - purple glow
		float cornerCircleSize = 70.0f + pulseAnimation * 20.0f;
		draw_list->AddCircleFilled(ImVec2(winPos.x, winPos.y), cornerCircleSize, 
			IM_COL32(80, 30, 120, 20), 32);
		draw_list->AddCircleFilled(ImVec2(winPos.x + winSize.x, winPos.y + winSize.y), cornerCircleSize, 
			IM_COL32(80, 30, 120, 20), 32);
		
		// Create custom title bar with glowing effect
		ImVec2 headerPos = ImVec2(winPos.x + 15, winPos.y + 15);
		ImGui::PushFont(globals::default_font.im_font);
		
		// No title bar background - removed as requested
		float extraWidth = 20.0f * scaleFactor; // Keep this for the separator line
		
		// Add a subtle line under where the title bar would be - purple glow
		draw_list->AddLine(
			ImVec2(winPos.x, winPos.y + 50 * scaleFactor),
			ImVec2(winPos.x + winSize.x, winPos.y + 50 * scaleFactor),
			IM_COL32(180, 100, 255, static_cast<int>((0.3f + pulseAnimation * 0.2f) * 255 * menuAlpha)),
			1.0f * scaleFactor
		);
		
		// Draw glowing text shadow for title - purple glow
		for (float i = 1; i <= 10; i += 2) {
			float alpha = (1.0f - i/10.0f) * 0.1f * (0.7f + pulseAnimation * 0.3f);
			draw_list->AddText(globals::default_font.im_font, 24.0f, 
				ImVec2(headerPos.x + i/2, headerPos.y + i/2), 
				IM_COL32(200, 100, 255, static_cast<int>(alpha * 255 * menuAlpha)), 
				"LOCKDOWN PROTOCOL");
		}
		
		// Draw main title
		draw_list->AddText(globals::default_font.im_font, 24.0f, headerPos, 
			IM_COL32(255, 255, 255, static_cast<int>(255 * menuAlpha)), "LOCKDOWN PROTOCOL");
		...
		draw_list->AddText(globals::default_font.im_font, 14.0f,
			...
			"v1.0 | Maintained by Zyhloh");
		
		// Add stylish X button in top right corner
		// Define the X button position and size
		const float xButtonSize = 24.0f;
		const float xButtonPadding = 12.0f;
		ImVec2 xButtonPos = ImVec2(winPos.x + winSize.x - xButtonSize - xButtonPadding, winPos.y + xButtonPadding);
		ImVec2 xButtonCenter = ImVec2(xButtonPos.x + xButtonSize/2, xButtonPos.y + xButtonSize/2);
		
		// Check if mouse is hovering over the X button
		ImVec2 mousePos = ImGui::GetMousePos();
		bool xButtonHovered = (mousePos.x >= xButtonPos.x && mousePos.x <= xButtonPos.x + xButtonSize &&
							 mousePos.y >= xButtonPos.y && mousePos.y <= xButtonPos.y + xButtonSize);
		
		// X button colors with hover effect - transparent background
		// No background color needed since we're making it transparent
		
		// X line color - brighter for better visibility against transparent background
		ImU32 xLineColor = xButtonHovered ? 
			IM_COL32(255, 255, 255, static_cast<int>(1.0f * 255 * menuAlpha)) : // White when hovered
			IM_COL32(220, 180, 255, static_cast<int>(0.9f * 255 * menuAlpha));  // Light purple normally
		
		// No need to draw background circle since it's transparent now
		
		// Draw the X lines - bolder
		float lineLength = xButtonSize * 0.35f; // Slightly larger X
		float lineThickness = xButtonHovered ? 3.0f : 2.5f; // Bolder lines
		draw_list->AddLine(
			ImVec2(xButtonCenter.x - lineLength, xButtonCenter.y - lineLength),
			ImVec2(xButtonCenter.x + lineLength, xButtonCenter.y + lineLength),
			xLineColor, lineThickness);
		draw_list->AddLine(
			ImVec2(xButtonCenter.x + lineLength, xButtonCenter.y - lineLength),
			ImVec2(xButtonCenter.x - lineLength, xButtonCenter.y + lineLength),
			xLineColor, lineThickness);
		...
		if (xButtonHovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
			// Exit the application using standard process termination
			exit(0);
		}
		
		ImGui::PopFont();
		
		// Set cursor position to start content right after the title bar
		ImGui::SetCursorPos(ImVec2(15, 60));
		
		// No need for an extra content area wrapper, we'll use the main content area directly

		// Custom animated cursor - purple theme
		auto cursor_position = util::cursor_position();
		// Inner circle
		ImGui::GetForegroundDrawList()->AddCircleFilled(
			ImVec2(cursor_position.x, cursor_position.y), 
			3.0f + pulseAnimation * 1.0f, 
			IM_COL32(255, 255, 255, 200));
		// Outer glowing circle
		for (float i = 0; i < 3; i++) {
			float size = 7.0f + i * 2.0f + pulseAnimation * 2.0f;
			float alpha = (1.0f - i/3.0f) * 0.6f;
			ImGui::GetForegroundDrawList()->AddCircle(
				ImVec2(cursor_position.x, cursor_position.y),
				size, IM_COL32(200, 120, 255, static_cast<int>(alpha * 255)), 0, 1.5f);
		}

		// Set control colors with animated glowing effect - purple theme
		ImVec4 baseAccentColor = ImVec4(0.5f, 0.2f, 0.7f, menuAlpha * 0.8f); // Purple with transparency
		ImVec4 glowAccentColor = ImVec4(
			baseAccentColor.x + pulseAnimation * 0.1f,
			baseAccentColor.y + pulseAnimation * 0.1f,
			baseAccentColor.z + pulseAnimation * 0.1f,
			baseAccentColor.w
		);
		
		ImGui::PushStyleColor(ImGuiCol_CheckMark, glowAccentColor);
		ImGui::PushStyleColor(ImGuiCol_SliderGrab, glowAccentColor);
		ImGui::PushStyleColor(ImGuiCol_SliderGrabActive, ImVec4(0.7f + pulseAnimation * 0.1f, 0.3f + pulseAnimation * 0.1f, 0.9f, menuAlpha * 0.8f));
		ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.15f, 0.08f, 0.2f, 0.8f * menuAlpha));
		ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.3f, 0.15f, 0.4f, 0.8f * menuAlpha));
		ImGui::PushStyleColor(ImGuiCol_ButtonActive, glowAccentColor);

		// Layout variables
		float calculatedHeight = 0.0f;
		float itemHeight = ImGui::GetFrameHeightWithSpacing();
		float headerPadding = 15.0f;
		float sidebarWidth = 140.0f; // Even wider sidebar for better appearance

		// Sidebar styling with animated effects
		ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 10.0f);
		ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 6.0f);
		
		// Animated sidebar background - purple theme with 50% transparency
		ImVec4 sidebarBgColor = ImVec4(0.1f, 0.05f, 0.15f, 0.5f * menuAlpha); // Dark purple with 50% transparency
		ImGui::PushStyleColor(ImGuiCol_ChildBg, sidebarBgColor);
		
		// Animated tab colors - purple theme
		ImVec4 headerColor = ImVec4(0.4f + pulseAnimation * 0.1f, 0.2f + pulseAnimation * 0.1f, 0.6f, (0.5f + pulseAnimation * 0.1f) * menuAlpha);
		ImVec4 headerHoveredColor = ImVec4(0.5f + pulseAnimation * 0.1f, 0.25f + pulseAnimation * 0.1f, 0.7f, (0.7f + pulseAnimation * 0.1f) * menuAlpha);
		ImVec4 headerActiveColor = ImVec4(0.6f + pulseAnimation * 0.1f, 0.3f + pulseAnimation * 0.1f, 0.8f, (0.9f + pulseAnimation * 0.1f) * menuAlpha);
		...
		// Begin sidebar with default padding
		ImGui::BeginChild("Sidebar", ImVec2(sidebarWidth, 0), true);
		
		// Main navigation tabs with centered text - moved up 10 pixels
		ImGui::PushFont(globals::default_font.im_font);
		// Skip the initial spacing to move buttons up
		
		// Simple approach with centered text using ImGui's built-in alignment
		const char* tabs[] = { "ESP", "PLAYER", "WEAPON", "ITEMS", "AIMBOT" };
		
		// Set up button style for full width - moved left 4 pixels
		ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 5));
		ImGui::PushStyleVar(ImGuiStyleVar_ItemInnerSpacing, ImVec2(0, 0));
		ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(0.5f, 0.5f));
		// Add left padding adjustment scaled to resolution
		ImGui::Indent(-7.0f * scaleFactor);
		
		for (int i = 0; i < 5; i++) {
			// Create a button that looks like a selectable but with better alignment
			ImGui::PushID(i);
			...
			// Create the button with full width - scaled for resolution
			if (ImGui::Button(tabs[i], ImVec2(sidebarWidth - 16 * scaleFactor, 30 * scaleFactor))) {
				selected_tab = i+1;
			}
		...
		// Menu hotkey - inline layout
		ImGui::Text("Menu Key");
		ImGui::SameLine();
		ImHotkey("##MenuHotkey", &menu_hotkey);
		ImGui::Spacing();
		
		// Config buttons with improved styling
		...
		// Debug section is hidden
		// Debug functionality is still available in the code but not shown in the UI

		ImGui::EndChild();

		ImGui::SameLine();
		// Begin the content area with fixed size and proper padding - no background
		ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0, 0, 0, 0)); // Transparent background
		ImGui::BeginChild("Content", ImVec2(windowSize.x - sidebarWidth - 50, windowSize.y - 80), false);
				...
				if (ImGui::BeginTabBar("ESPTabBar")) {
					...
					if (ImGui::BeginTabItem("Weapon")) {
						...
						if (ImGui::CollapsingHeader("Details##WeaponESPDetails", ImGuiTreeNodeFlags_DefaultOpen)) {
							...
							ImGui::Checkbox("Hide Empty Weapons##HideEmptyWeapons", &hide_empty_weapons);
